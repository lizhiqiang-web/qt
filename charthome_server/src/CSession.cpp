...
void CSession::setUeserName(string user_name){
	_user_name = user_name;
}
string& CSession::getUeserName(){
	return _user_name;
}
...
void CSession::OverHandle(){
	Json::Value root;
	root["id"] = 999;
	root["del_name"] = _user_name;
	root["data"] =_user_name + "@退出聊天室";
	string js_data = root.toStyledString();
	auto sessions = _server->GetSessions();
	for(auto it = sessions.begin(); it != sessions.end(); it++)
	{	
		
		it->second->Send(js_data,999);
	
	}
}
void CSession::StartHandle(){
	Json::Value root;
	root["id"] = 998;
	root["add_name"] = _user_name;
	root["data"] =_user_name + "@加入聊天室";
	string js_data = root.toStyledString();
	...
	auto temp = sessions.end();
	temp--;
	for(auto it = sessions.begin(); it != sessions.end(); it++)
	{	
		if(it->first != this->_uuid)
		it->second->Send(js_data,998);
		
		if(it == temp)
		{
			names += it->second->_user_name; 
		}
		else
		{
			names += it->second->_user_name + ","; 
	...
	root["id"] = 997;
	root["names"] = names;
	js_data = root.toStyledString();
	this->Send(js_data,997);
}
